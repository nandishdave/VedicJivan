# -*- coding: utf-8 -*-
"""Candidate I - ATMAKARAKA strength as factor 16 vs the current 15-factor model.
The Atmakaraka (AK, Jaimini) = the planet with the highest degree-in-sign among the 7
classical grahas — the 'soul' significator. We tested Karakamsa YOGAS (rejected); here we
test simply whether the AK is STRONG in its own right. Metrics:
  ak_dign = D1 dignity of the AK        ak_vim = Vimsopaka (16-varga) of the AK
  ak_dig  = digbala of the AK           ak_sav = SAV bindus of the AK's sign
  ak_shad = total shadbala of the AK
...
MKEYS = ["ak_dign", "ak_vim", "ak_dig", "ak_sav", "ak_shad"]
...
def feats(p):
    ...
    vim_avg = float(np.mean([_vim(q, P, dv) for q in _C]))
    ...
    # ---- CANDIDATE I: Atmakaraka strength ----
    ak = calc_jaimini_karakas(P, lag)["atmakaraka"]
    ak_dign = float(_DP.get(_get_dignity(ak, P[ak]["sign"]), 45))
    ak_vim = float(_vim(ak, P, dv))
    ak_dig = float(db(ak))
    ak_sav = float(tv[P[ak]["sign"]])
    ak_shad = float(sb[ak]["total_shadbala"]) if ak in sb else avg
    strength = [ak_dign, ak_vim, ak_dig, ak_sav, ak_shad]
...
for i, k in enumerate(MKEYS):
    print(f"  {k:9} fam={FS[:,i].mean():7.2f} ord={RS[:,i].mean():7.2f}  soloAUC={auc(FS[:,i], RS[:,i]):.3f}")
