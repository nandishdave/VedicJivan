# -*- coding: utf-8 -*-
"""Re-check the numerology placement rule against the CURRENT 14-factor model.
Fixed {1,2,3,5,10} + HONEST nested (house-set re-picked per fold). Is it still
redundant, or does the 14-factor model leave it a real marginal lift? (225/96)"""
...
def feats(p):
    ...
    mh, bh = P[NUMP[mool]]["house"], P[NUMP[bhag]]["house"]
    ph = np.zeros(12); ph[mh - 1] += 1; ph[bh - 1] += 1     # occupancy of the 2 numerology planets by house
    return f14, mh, bh, ph, by, (68 <= b["lon"] <= 98 and 6 <= b["lat"] <= 37)
...
PH = np.vstack([np.array([x[3] for x in FR]), np.array([x[3] for x in RR])])
X14 = np.vstack([F14, R14]); Y = np.array([1] * len(FR) + [0] * len(RR), float)
...
def cv_nested(topk=5):
    ...
    for i in range(5):
        ...
        sel = set(int(h) for h in np.argsort(diffs)[::-1][:topk]); picks.append(sorted(h + 1 for h in sel))
        feat = np.array([(m - 1 in sel) + (b - 1 in sel) for m, b in zip(MH, BH)], float)
        X = np.column_stack([X14, feat]); sg = np.sign(X[tr][Y[tr] == 1].mean(0) - X[tr][Y[tr] == 0].mean(0)); m, sd = X[tr].mean(0), X[tr].std(0) + 1e-9
        s[te] = (((X[te] - m) / sd) * sg).sum(1)
    return auc(s[Y == 1], s[Y == 0]), picks
fixed = np.array([(m in SEL0) + (b in SEL0) for m, b in zip(MH, BH)], float)
Ff, Rf = fixed[:len(FR)], fixed[len(FR):]
print(f"famous={len(FR)} ordinary={len(RR)}   numerology placement re-check vs 14-factor")
print("base 14-factor:            " + "  ".join(f"{k}={cv(F14[IDX[k]],R14):.3f}" for k in IDX))
F = np.column_stack([F14, Ff]); R = np.column_stack([R14, Rf])
print("+ fixed {1,2,3,5,10} (IN-SAMPLE): " + "  ".join(f"{k}={cv(F[IDX[k]],R):.3f}" for k in IDX) + f"   solo-AUC={auc(Ff,Rf):.3f}")
a5, p5 = cv_nested(5)
print(f"+ nested top-5/fold (HONEST):     full={a5:.3f}   picks={p5}")
