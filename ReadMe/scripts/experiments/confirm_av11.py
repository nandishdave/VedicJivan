"""Confirm the 8-factor composite (7 winners + av_11th, no d10): full + matched
cuts, and capture av_11th's (fam_mean, ord_mean, pooled_std) for the REF.
Uses with_shadbala=False (none of these factors need Shadbala) -> fast."""
import json
import numpy as np
...
def feats(dob, tob, lat, lon):
    ...
    return [rahu, d60c, av10, av1, upa, _act(dl, by, _pw(raja_yoga_score(c)[1]), 50, 80),
            _act(dl, by, _pw(dhana_yoga_score(c)[1]), 50, 80), av11], by, (68 <= lon <= 98 and 6 <= lat <= 37)
...
c7, s7 = cv(list(range(7))); c8, s8 = cv(list(range(8)))
print(f"7-factor:  count-AUC={c7:.3f}  sum-AUC={s7:.3f}")
print(f"8-factor:  count-AUC={c8:.3f}  sum-AUC={s8:.3f}")
print("matched-India cuts (sum-AUC): 7f -> 8f")
for yr in (0, 1940, 1955):
    mask = (FY >= yr) & FI
    if mask.sum() < 20: continue
    _, a7 = cv(list(range(7)), F[mask], R); _, a8 = cv(list(range(8)), F[mask], R)
    print(f"  India >= {yr:4d}  n={mask.sum():3d}   {a7:.3f} -> {a8:.3f}")
ALL = np.vstack([F, R])
print(f'\nREF av_11th: ({F[:,7].mean():.4f}, {R[:,7].mean():.4f}, {ALL[:,7].std(ddof=1):.4f})')
