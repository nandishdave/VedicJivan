# -*- coding: utf-8 -*-
"""Candidate C - DIGBALA (directional strength) as factor 15. Uses the engine's real
classical dig_bala (0-60 virupas). Fame hypothesis: prominence planets in their power
directions -> Sun/Mars strong in the 10th (authority/action, South), Jupiter/Mercury
in the 1st (the person shines, East). Metrics:
  dig_sun    = Sun's digbala (the 'king'/fame planet)
  dig_mars   = Mars's digbala
  dig_fame4  = Sun+Mars+Jupiter+Mercury digbala (the four prominence planets)
  dig_all    = mean digbala of all 7
  dig_10lord = digbala of the 10th lord (career lord's directional strength)
  dig_laglrd = digbala of the lagna lord
Solo AUC + fixed adds + HONEST nested best-of-6/fold + seed-stability. (225 vs 96)"""
...
MKEYS = ["dig_sun", "dig_mars", "dig_fame4", "dig_all", "dig_10lord", "dig_laglrd"]
...
def feats(p):
    ...
    dig_sun = db("Sun"); dig_mars = db("Mars")
    dig_fame4 = dig_sun + dig_mars + db("Jupiter") + db("Mercury")
    dig_all = float(np.mean([db(q) for q in _C]))
    dig_10lord = db(SIGN_LORDS[(ls + 9) % 12]); dig_laglrd = db(SIGN_LORDS[ls])
    strength = [dig_sun, dig_mars, dig_fame4, dig_all, dig_10lord, dig_laglrd]
...
for i, k in enumerate(MKEYS):
    print(f"  {k:11} fam={FS[:,i].mean():7.2f} ord={RS[:,i].mean():7.2f}  soloAUC={auc(FS[:,i], RS[:,i]):.3f}")
print("\nbase 14-factor:            " + "  ".join(f"{k}={cv(F[CUTS[k]],R):.3f}" for k in CUTS))
for i, k in enumerate(MKEYS):
    Fn, Rn = add([i]); print(f"+ {k:11}             " + "  ".join(f"{ck}={cv(Fn[CUTS[ck]],Rn):.3f}" for ck in CUTS))
...
print(f"\n+ nested best-of-6/fold (HONEST): full={auc(s[Y==1],s[Y==0]):.3f}  picks={picks}")
bi = int(np.argmax([max(auc(FS[:, i], RS[:, i]), 1 - auc(FS[:, i], RS[:, i])) for i in range(len(MKEYS))]))
Fn, Rn = add([bi]); print(f"seed-stability +{MKEYS[bi]}:", [f"{cv(Fn,Rn,seed=sd):.3f}" for sd in [1,7,42,123,2024]])
print("\n--- deciding: per-cut seed-stability of the correctly-signed lord-digbala metrics ---")
SEEDS = [1, 7, 42, 123, 2024]
for name, idxs in [("dig_laglrd", [5]), ("dig_10lord", [4]), ("both", [4, 5])]:
    Fn, Rn = add(idxs)
    print(f"  +{name}:")
    for ck in CUTS:
...
# nested restricted to the two correctly-signed lord metrics only
S2 = S[:, [4, 5]]
np.random.seed(7); idx = np.random.permutation(len(Y)); folds = np.array_split(idx, 5); s = np.zeros(len(Y)); picks = []
for i in range(5):
    te = folds[i]; tr = np.concatenate([folds[j] for j in range(5) if j != i])
    seps = [abs(S2[tr][Y[tr] == 1, m].mean() - S2[tr][Y[tr] == 0, m].mean()) / (S2[tr][:, m].std() + 1e-9) for m in range(2)]
    bm = int(np.argmax(seps)); picks.append(["dig_10lord", "dig_laglrd"][bm])
    Xx = np.column_stack([X, S2[:, bm]]); sg = np.sign(Xx[tr][Y[tr] == 1].mean(0) - Xx[tr][Y[tr] == 0].mean(0)); m_, sd_ = Xx[tr].mean(0), Xx[tr].std(0) + 1e-9
    s[te] = (((Xx[te] - m_) / sd_) * sg).sum(1)
print(f"\n+ nested best-of-2 lord-digbala/fold (HONEST): full={auc(s[Y==1],s[Y==0]):.3f}  picks={picks}")
for i in (4, 5):
    allv = np.concatenate([FS[:, i], RS[:, i]]); print(f"REF {MKEYS[i]}: ({FS[:,i].mean():.4f}, {RS[:,i].mean():.4f}, {allv.std(ddof=1):.4f})")
# SINGLE combined factor = mean digbala of lagna-lord & 10th-lord (the productionizable form)
FL = (FS[:, 4] + FS[:, 5]) / 2.0; RL = (RS[:, 4] + RS[:, 5]) / 2.0
print(f"\n--- SINGLE factor dig_lords = mean(laglord, 10lord) digbala ---  soloAUC={auc(FL,RL):.3f}")
Fn = np.column_stack([F, FL]); Rn = np.column_stack([R, RL])
SEEDS = [1, 7, 42, 123, 2024]
for ck in CUTS:
    bvals = [cv(F[CUTS[ck]], R, seed=sd) for sd in SEEDS]; svals = [cv(Fn[CUTS[ck]], Rn, seed=sd) for sd in SEEDS]
    print(f"  {ck:7} base={np.mean(bvals):.3f}  +dig_lords={np.mean(svals):.3f}  delta={np.mean(svals)-np.mean(bvals):+.3f}  {[f'{v:.3f}' for v in svals]}")
allv = np.concatenate([FL, RL]); print(f"REF dig_lords: ({FL.mean():.4f}, {RL.mean():.4f}, {allv.std(ddof=1):.4f})")
# combined with A's swap already in? quick: swap D60->? not here; that's step 6. Report independ& corr with existing.
print(f"corr(dig_lords, moon_disp)={np.corrcoef(np.concatenate([F[:,9],R[:,9]]), np.concatenate([FL,RL]))[0,1]:+.3f}  corr(dig_lords, av1)={np.corrcoef(np.concatenate([F[:,3],R[:,3]]), np.concatenate([FL,RL]))[0,1]:+.3f}")
