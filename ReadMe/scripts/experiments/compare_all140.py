"""Full recalculation on the 140-celebrity set vs 96 ordinary: 8-factor, 9-factor
(+ NATAL age-free Rahu Yoga), and the winners composite. Count and sum."""
import json
import numpy as np
...
_BAD={3,6,8,12}
FEAT=["dasha","dhana","prosperity","raja","func_ben","av_1st","av_10th","d60","rahu_natal"]
...
def feats(dob,tob,lat,lon):
    ...
    def ds(pl):   # current 2x2 node dasha (as in the 8/9-factor model)
        if pl in ("Rahu","Ketu"):
    ...
    return [fd,dhana_yoga_score(c)[0],prosperity_yoga_score(c)[0],raja_yoga_score(c)[0],fbc,tv[ls],tv[(ls+9)%12],fd60,natal*dispf]
...
print()
report(list(range(8)),"8-factor")
report(list(range(9)),"9-factor (+ Rahu Yoga natal)")
report([N["rahu_natal"],N["d60"],N["av_10th"],N["av_1st"]],"WINNERS (rahu_natal+d60+10AV+1AV)")
