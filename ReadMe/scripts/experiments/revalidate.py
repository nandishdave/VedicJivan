# -*- coding: utf-8 -*-
"""Re-validate the 11-factor with moon_disp = dispositor in {1,2,11,12}. Full +
matched cuts + the exact REF for the widened factor. (225 vs 96)"""
import json
import numpy as np
...
_BAD = {3, 6, 8, 12}; _OCC = {3, 6, 10, 11}; DISP_SET = {1, 2, 11, 12}
FEAT = ["rahu_prime", "d60", "av_10th", "av_1st", "upa_occ", "raja_late", "dhana_late", "av_11th", "bright_moon", "moon_disp", "moon_sav"]
...
def feats(p):
    ...
    moon_disp = 1.0 if P[SIGN_LORDS[ms]]["house"] in DISP_SET else 0.0
    moon_sav = tv[ms]
    return [rahu, d60c, av10, av1, upa, rl, dhl, av11, bright, moon_disp, moon_sav], by, (68 <= b["lon"] <= 98 and 6 <= b["lat"] <= 37)
...
cc, ss = cv(F, R)
print(f"11-factor (moon_disp in 1/2/11/12):  count-AUC={cc:.3f}  sum-AUC={ss:.3f}")
print("matched-India cuts (sum-AUC):")
for yr in (0, 1940, 1955):
    mask = (FY >= yr) & FI
    if mask.sum() < 20: continue
    print(f"  India >= {yr:4d}  n={mask.sum():3d}   {cv(F[mask], R)[1]:.3f}")
ALL = np.vstack([F, R]); i = 9
print(f'\nREF moon_disp: ({F[:,i].mean():.4f}, {R[:,i].mean():.4f}, {ALL[:,i].std(ddof=1):.4f})')
