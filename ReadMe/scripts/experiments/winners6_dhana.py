"""Does Dhana yoga (lords of 1/2/11) belong in the rules?
Recompute Dhana factor on 207-vs-96, its solo AUC, and 5-factor vs 6-factor CV-AUC."""
import json
import numpy as np
...
FEAT = ["rahu_natal", "d60_crude", "av_10th", "av_1st", "upa_occ", "dhana"]
...
def feats(dob, tob, lat, lon):
    ...
    dhana = dhana_yoga_score(c)[0]
    return [rahu_natal, d60c, av10, av1, upa_occ, dhana], by, (68 <= lon <= 98 and 6 <= lat <= 37)
...
dh_all = np.concatenate([F[:, 5], R[:, 5]])
print(f"\n  Dhana yoga SOLO factor AUC = {auc(dh_all, yv):.3f}")
c5, s5 = cv(F[:, :5], R[:, :5]); print(f"  5-factor (verified)              count-AUC={c5:.3f}  sum-AUC={s5:.3f}")
c6, s6 = cv(F[:, :6], R[:, :6]); print(f"  6-factor (+ Dhana yoga)          count-AUC={c6:.3f}  sum-AUC={s6:.3f}")
