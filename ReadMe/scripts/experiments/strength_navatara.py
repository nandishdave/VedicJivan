# -*- coding: utf-8 -*-
"""Candidate factor 17 - NAVATARA CHAKRA. From the Janma nakshatra (Moon's), each planet's
tara = ((planet_nak - moon_nak) mod 9)+1. Benefic taras {2 Sampat,4 Kshema,6 Sadhaka,
8 Mitra,9 Atimitra}, malefic {3 Vipat,5 Pratyari,7 Vadha}, neutral {1 Janma}. Do famous
have more planets in benefic taras? Test vs the current 16-factor model. (225 vs 96)"""
...
SEAT = {1, 2, 4, 5, 11}
_BEN_TARA = {2, 4, 6, 8, 9}; _MAL_TARA = {3, 5, 7}
_NBODIES = ["Sun", "Mars", "Mercury", "Jupiter", "Venus", "Saturn"]  # exclude Moon (defines Janma)
NAK = 360.0 / 27.0
def nak_of(lon): return int((lon % 360) / NAK)
...
def feats(p):
    ...
    # ---- CANDIDATE 17: Navatara ----
    jn = nak_of(P["Moon"]["longitude"])
    taras = {q: ((nak_of(P[q]["longitude"]) - jn) % 9) + 1 for q in _NBODIES + ["Rahu", "Ketu"]}
    ben6 = float(sum(1 for q in _NBODIES if taras[q] in _BEN_TARA))
    mal6 = float(sum(1 for q in _NBODIES if taras[q] in _MAL_TARA))
    net6 = ben6 - mal6
    ben8 = float(sum(1 for q in _NBODIES + ["Rahu", "Ketu"] if taras[q] in _BEN_TARA))
    mal8 = float(sum(1 for q in _NBODIES + ["Rahu", "Ketu"] if taras[q] in _MAL_TARA))
    net8 = ben8 - mal8
    strength = [ben6, mal6, net6, net8]
    return f16, [float(x) for x in strength], by, (68 <= b["lon"] <= 98 and 6 <= b["lat"] <= 37)
...
def add(i):
    return np.column_stack([F, FS[:, i]]), np.column_stack([R, RS[:, i]])
print(f"famous={len(FR)} ordinary={len(RR)}  (Moon excluded; 6 planets ±nodes)")
print("\nSolo separation (fam_mean, ord_mean, solo-AUC):")
for i, k in enumerate(MKEYS):
    print(f"  {k:6} fam={FS[:,i].mean():6.3f} ord={RS[:,i].mean():6.3f}  soloAUC={auc(FS[:,i], RS[:,i]):.3f}")
print("\nbase 16-factor:            " + "  ".join(f"{k}={cv(F[CUTS[k]],R):.3f}" for k in CUTS))
for i, k in enumerate(MKEYS):
    Fn, Rn = add(i); print(f"+ {k:6}                  " + "  ".join(f"{ck}={cv(Fn[CUTS[ck]],Rn):.3f}" for ck in CUTS))
...
print(f"\n+ nested best-of-4/fold (HONEST): full={auc(s[Y==1],s[Y==0]):.3f}  picks={picks}")
