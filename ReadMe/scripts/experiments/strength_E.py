# -*- coding: utf-8 -*-
"""Candidate E - FAME-FROM-THE-MOON special points as factor 15.
INDU LAGNA (wealth point, well-defined): kala{Sun30,Moon16,Mars6,Mer8,Jup10,Ven12,Sat1};
  total = kala[9th-lord-from-lagna] + kala[9th-lord-from-Moon]; rem = total%12 (12 if 0);
  Indu Lagna sign = rem-th sign FROM the Moon (inclusive).
SREE LAGNA (prosperity, approx sign-fraction form): SL_long = lagna_long + frac*360,
  frac = (moon_long mod 30)/30.
Metrics: benefics/planets in wealth houses (2/11) & kendra/trikona from IL, IL SAV bindus,
IL-lord condition; + SL kendra/trikona benefics & SL SAV. Solo + factor-15 + nested/seed. (225 vs 96)"""
...
KALA = {"Sun": 30, "Moon": 16, "Mars": 6, "Mercury": 8, "Jupiter": 10, "Venus": 12, "Saturn": 1}
KT = {1, 4, 5, 7, 9, 10}
MKEYS = ["il_occ", "il_2_11", "il_kt_ben", "il_sav", "il_lord", "sl_kt_ben", "sl_sav"]
...
def feats(p):
    ...
    ms = P["Moon"]["sign"]; sl_lon, ml = P["Sun"]["longitude"], P["Moon"]["longitude"]; elong = (ml - sl_lon) % 360
    ...
    # ---- CANDIDATE E: Indu Lagna + Sree Lagna ----
    l9_lag = SIGN_LORDS[(ls + 8) % 12]; l9_moon = SIGN_LORDS[(ms + 8) % 12]
    total = KALA[l9_lag] + KALA[l9_moon]; rem = total % 12 or 12
    IL = (ms + rem - 1) % 12
    ben = {"Jupiter", "Venus", "Mercury"} | ({"Moon"} if bright else set())
    hIL = lambda s: ((s - IL) % 12) + 1
    il_occ = float(sum(1 for q in _ALL if P[q]["sign"] == IL))
    il_2_11 = float(sum(1 for q in _C if hIL(P[q]["sign"]) in {2, 11}))
    il_kt_ben = float(sum(1 for q in _C if q in ben and hIL(P[q]["sign"]) in KT))
    il_sav = float(tv[IL])
    ill = SIGN_LORDS[IL]; illh = ((P[ill]["sign"] - ls) % 12) + 1
    il_lord = float((_get_dignity(ill, P[ill]["sign"]) in _DIGN) + (illh in KT))
    lag_lon = ls * 30 + lag["degree"]; frac = (ml % 30) / 30.0; SL = int((lag_lon + frac * 360) % 360 / 30) % 12
    hSL = lambda s: ((s - SL) % 12) + 1
    sl_kt_ben = float(sum(1 for q in _C if q in ben and hSL(P[q]["sign"]) in KT))
    sl_sav = float(tv[SL])
    strength = [il_occ, il_2_11, il_kt_ben, il_sav, il_lord, sl_kt_ben, sl_sav]
...
for i, k in enumerate(MKEYS):
    ...
    print(f"+ {k:10}              " + "  ".join(f"{ck}={cv(Fn[CUTS[ck]],Rn):.3f}" for ck in CUTS))
...
print(f"\n+ nested best-of-7/fold (HONEST): full={auc(s[Y==1],s[Y==0]):.3f}  picks={picks}")
bi = int(np.argmax([abs(auc(FS[:, i], RS[:, i]) - 0.5) for i in range(len(MKEYS))]))
Fn = np.column_stack([F, FS[:, bi]]); Rn = np.column_stack([R, RS[:, bi]]); SEEDS = [1, 7, 42, 123, 2024]
print(f"\nseed-stability best ({MKEYS[bi]}):")
for ck in CUTS:
