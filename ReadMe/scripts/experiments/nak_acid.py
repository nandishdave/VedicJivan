# -*- coding: utf-8 -*-
"""ACID TEST for Factor 17 (Mridu count): re-derive which nakshatra QUALITY separates
best INSIDE each training fold, then score held-out charts. If Mridu is genuinely
predictive (not cherry-picked from 7 qualities), a per-fold-selected version still lifts.
Reuses the 16-factor base + the 7 per-body quality counts (9 bodies). (225/96)"""
...
QF = np.array([x[1] for x in FR]); QR = np.array([x[1] for x in RR])  # 7 quality counts
...
def nested_cv(seed):
    # per fold: pick the quality with max |sep| in TRAIN, add its count as a feature, score held-out
    Xall = X; Qall = Q; y = Y
    np.random.seed(seed); idx = np.random.permutation(len(y)); folds = np.array_split(idx, 5); s = np.zeros(len(y)); picks=[]
    for i in range(5):
        te = folds[i]; tr = np.concatenate([folds[j] for j in range(5) if j != i])
        seps = [abs(Qall[tr][y[tr]==1,m].mean()-Qall[tr][y[tr]==0,m].mean())/(Qall[tr][:,m].std()+1e-9) for m in range(7)]
        bm = int(np.argmax(seps)); picks.append(QORDER[bm])
        Xx = np.column_stack([Xall, Qall[:,bm]]); sg=np.sign(Xx[tr][y[tr]==1].mean(0)-Xx[tr][y[tr]==0].mean(0))
...
print("\n=== ACID TEST: quality re-derived per fold (full set) ===")
base = np.mean([base_cv(CUTS['full'],sd) for sd in SEEDS])
vals=[];
for sd in SEEDS:
    a,pk=nested_cv(sd); vals.append(a)
print(f"  base full (5-seed):        {base:.3f}")
print(f"  nested-quality full (5s):  {np.mean(vals):.3f}  {[f'{v:.3f}' for v in vals]}  delta={np.mean(vals)-base:+.3f}")
_,pk7=nested_cv(7); print(f"  per-fold quality picks (seed7): {pk7}")
