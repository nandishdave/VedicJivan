# -*- coding: utf-8 -*-
"""Nested-validate the Arudha Lagna factor-15 candidate. Pre-committed al_lagna_kt
(14+1), 3-metric AL bundle (14+3), nested best-of-6-per-fold (kills selection
optimism), seed-stability, + REF for productionizing. (225 vs 96)"""
...
ALKEYS = ["al_2_11_ben", "al_kt_ben", "al_occ", "al_2_10_11", "al_lagna_kt", "al_lord_strong"]
def arudha_lagna(ls, P):
    lord = SIGN_LORDS[ls]; lsgn = P[lord]["sign"]; d = (lsgn - ls) % 12; al = (lsgn + d) % 12
    if al == ls or al == (ls + 6) % 12: al = (al + 9) % 12
    return al
...
def feats(p):
    ...
    hal = lambda s: ((s - al) % 12) + 1
    almet = [
        sum(1 for q in _C if q in ben and hal(P[q]["sign"]) in {2, 11}),
        sum(1 for q in _C if q in ben and hal(P[q]["sign"]) in KT),
        sum(1 for q in _ALL if hal(P[q]["sign"]) == 1),
        sum(1 for q in _ALL if hal(P[q]["sign"]) in {2, 10, 11}),
        1.0 if (((al - ls) % 12) + 1) in KT else 0.0,
        1.0 if (_get_dignity(SIGN_LORDS[al], P[SIGN_LORDS[al]]["sign"]) in _DIGN or P[SIGN_LORDS[al]]["house"] in KEND) else 0.0,
    ]
    return f14, [float(x) for x in almet], by, (68 <= b["lon"] <= 98 and 6 <= b["lat"] <= 37)
FR = [feats(p) for p in FAM]; RR = [feats(p) for p in ORDD]
F = np.array([x[0] for x in FR]); R = np.array([x[0] for x in RR])
FAL = np.array([x[1] for x in FR]); RAL = np.array([x[1] for x in RR])
X = np.vstack([F, R]); AL = np.vstack([FAL, RAL]); Y = np.array([1] * len(FR) + [0] * len(RR), float)
...
def add(cols):
    Fn = np.column_stack([F] + [FAL[:, i] for i in cols]); Rn = np.column_stack([R] + [RAL[:, i] for i in cols])
    return Fn, Rn
print(f"famous={len(FR)} ordinary={len(RR)}")
print("base 14-factor:            " + "  ".join(f"{k}={cv(F[CUTS[k]],R):.3f}" for k in CUTS))
Fn, Rn = add([4]); print("+ al_lagna_kt (pre-committed): " + "  ".join(f"{k}={cv(Fn[CUTS[k]],Rn):.3f}" for k in CUTS))
Fn, Rn = add([0, 3, 4]); print("+ AL bundle {2/11ben,2/10/11,lagna_kt}: " + "  ".join(f"{k}={cv(Fn[CUTS[k]],Rn):.3f}" for k in CUTS))
# nested best-of-6 per fold (full set)
np.random.seed(7); idx = np.random.permutation(len(Y)); folds = np.array_split(idx, 5); s = np.zeros(len(Y)); picks = []
for i in range(5):
    te = folds[i]; tr = np.concatenate([folds[j] for j in range(5) if j != i])
    seps = [abs(AL[tr][Y[tr] == 1, m].mean() - AL[tr][Y[tr] == 0, m].mean()) / (AL[tr][:, m].std() + 1e-9) for m in range(6)]
    bm = int(np.argmax(seps)); picks.append(ALKEYS[bm])
    Xx = np.column_stack([X, AL[:, bm]]); sg = np.sign(Xx[tr][Y[tr] == 1].mean(0) - Xx[tr][Y[tr] == 0].mean(0)); m_, sd_ = Xx[tr].mean(0), Xx[tr].std(0) + 1e-9
    s[te] = (((Xx[te] - m_) / sd_) * sg).sum(1)
print(f"+ nested best-of-6/fold (HONEST): full={auc(s[Y==1],s[Y==0]):.3f}  picks={picks}")
# seed stability of al_lagna_kt
Fn, Rn = add([4]); seeds = [1, 7, 42, 123, 2024]
print("seed-stability al_lagna_kt full-AUC:", [f"{cv(Fn,Rn,seed=sd):.3f}" for sd in seeds])
alk_f = FAL[:, 4]; alk_r = RAL[:, 4]; allv = np.concatenate([alk_f, alk_r])
print(f"REF al_lagna_kt: ({alk_f.mean():.4f}, {alk_r.mean():.4f}, {allv.std(ddof=1):.4f})   solo-AUC={auc(alk_f,alk_r):.3f}")
