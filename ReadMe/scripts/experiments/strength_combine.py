# -*- coding: utf-8 -*-
"""Should Vimsopaka (magnitude, factor 2) + seat (position, factor 16) stay SEPARATE, or
combine into ONE 'Vimsopaka-in-good-houses' factor? Test model variants vs the same base-14
(the 16 minus vim_mean minus seat). count/sum CV-AUC, full + matched cuts. (225 vs 96)"""
...
def feats(p):
    ...
    vim_mean = float(np.mean(list(vim.values())))
    ...
    # candidate pieces
    top = max(_C, key=lambda q: vim[q]); th = P[top]["house"]
    seat_211 = 1.0 if th in (1, 2, 11) else 0.0
    seat_12451 = 1.0 if th in (1, 2, 4, 5, 11) else 0.0
    vim_in_211 = sum(vim[q] for q in _C if P[q]["house"] in (1, 2, 11))       # summed strength in prominence houses
    vim_in_12451 = sum(vim[q] for q in _C if P[q]["house"] in (1, 2, 4, 5, 11))
    HW = {1: 1.0, 2: 1.0, 11: 1.0, 4: 0.5, 5: 0.5, 3: 0.0, 7: 0.0, 9: 0.0, 12: 0.0, 6: -1.0, 8: -1.0, 10: -1.0}
    vim_wtd = sum(vim[q] * HW[P[q]["house"]] for q in _C)                     # strength weighted by house-goodness
    extra = [vim_mean, seat_211, seat_12451, vim_in_211, vim_in_12451, vim_wtd]
...
EK = {"vim_mean": 0, "seat_211": 1, "seat_12451": 2, "vim_in_211": 3, "vim_in_12451": 4, "vim_wtd": 5}
def auc(fa, ra): return float(np.mean([np.mean(x > ra) + 0.5 * np.mean(x == ra) for x in fa]))
def cv(cols, cut, seed=7, mode="sum"):
    ...
    for i in range(5):
        ...
        Z = ((X[te]-m)/sd)*sg; s[te] = (Z > 0).sum(1) if mode == "count" else Z.sum(1)
    return auc(s[y==1], s[y==0])
SEEDS = [1, 7, 42, 123, 2024]
def line(name, cols, nf):
    row = "  ".join(f"{k}={cv(cols, CUTS[k]):.3f}" for k in CUTS)
    m5 = np.mean([cv(cols, CUTS['full'], seed=sd) for sd in SEEDS])
    print(f"  {name:42} [{nf}f] {row}   full-5seed={m5:.3f}")
print(f"famous={len(FR)} ordinary={len(RR)}\nModel variants (sum-AUC, seed 7 + 5-seed full):")
line("A  base14 + vim_mean + seat{1,2,11}  (CURRENT)", ["vim_mean", "seat_211"], 16)
line("B  base14 + vim_mean only            (pre-16)", ["vim_mean"], 15)
line("C  base14 + vim_in{1,2,11}  (COMBINED, single)", ["vim_in_211"], 15)
line("D  base14 + vim_in{1,2,4,5,11} (COMBINED single)", ["vim_in_12451"], 15)
line("E  base14 + vim_wtd house-weighted (COMBINED)", ["vim_wtd"], 15)
line("F  base14 + vim_mean + vim_in{1,2,4,5,11}", ["vim_mean", "vim_in_12451"], 16)
line("G  base14 + vim_mean + seat{1,2,4,5,11}", ["vim_mean", "seat_12451"], 16)
print("\nsolo AUC of the combined single-factor forms:")
for k in ["vim_in_211", "vim_in_12451", "vim_wtd"]:
    print(f"  {k:14} soloAUC={auc(E_F[:, EK[k]], E_R[:, EK[k]]):.3f}   corr(vim_mean)={np.corrcoef(np.concatenate([E_F[:,0],E_R[:,0]]), np.concatenate([E_F[:,EK[k]],E_R[:,EK[k]]]))[0,1]:+.3f}")
