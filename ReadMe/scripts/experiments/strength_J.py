# -*- coding: utf-8 -*-
"""Candidate J - D9 (NAVAMSA) IMPORTANCE, functional-benefic-specific, as factor 16 vs the
current 15-factor model. Distinct from vim_avg (which averages D9 dignity over ALL 7 planets
generically). Functional benefics = trikona lords (1st/5th/9th lords), deduped. Metrics:
  fb_d9_dign  = mean D9 dignity of the functional benefics
  fb_d9_strong= # of FBs with D9 dignity >= Friendly (55)
  fb_d9_exown = # of FBs Exalted/Moolatrikona/Own in D9
  fb_exch     = # of parivartana (mutual-sign exchange) pairs among FBs IN D9
  d9lag_good  = D9-lagna sign lands in a good house {1,2,4,5,9,10,11} from the D1 lagna
  d9lag_lord  = the D9-lagna-sign's lord sits in a good D1 house {1,2,4,5,9,10,11}
Solo + factor-16 + nested/seed. (225 vs 96)"""
...
def feats(p):
    ...
    # ---- CANDIDATE J: D9 / functional benefics ----
    D9 = dv["D9"]
    fb = list({SIGN_LORDS[(ls + h) % 12] for h in (0, 4, 8)})  # trikona (1/5/9) lords, deduped
    d9dig = [_get_dignity(q, D9[q]) for q in fb]
    fb_d9_dign = float(np.mean([_DP.get(x, 45) for x in d9dig]))
    fb_d9_strong = float(sum(1 for x in d9dig if x in _STRONG))
    fb_d9_exown = float(sum(1 for x in d9dig if x in _EXOWN))
    fb_exch = 0.0
    for i in range(len(fb)):
        for j in range(i + 1, len(fb)):
            if SIGN_LORDS[D9[fb[i]]] == fb[j] and SIGN_LORDS[D9[fb[j]]] == fb[i]: fb_exch += 1
    d9lag = D9["Lagna"]
    d9lag_good = 1.0 if (((d9lag - ls) % 12) + 1) in _GOOD else 0.0
    d9lag_lord = 1.0 if P[SIGN_LORDS[d9lag]]["house"] in _GOOD else 0.0
    strength = [fb_d9_dign, fb_d9_strong, fb_d9_exown, fb_exch, d9lag_good, d9lag_lord]
...
for i, k in enumerate(MKEYS):
    print(f"  {k:13} fam={FS[:,i].mean():6.3f} ord={RS[:,i].mean():6.3f}  soloAUC={auc(FS[:,i], RS[:,i]):.3f}")
print("\nbase 15-factor:            " + "  ".join(f"{k}={cv(F[CUTS[k]],R):.3f}" for k in CUTS))
for i, k in enumerate(MKEYS):
    Fn, Rn = add([i]); print(f"+ {k:13}           " + "  ".join(f"{ck}={cv(Fn[CUTS[ck]],Rn):.3f}" for ck in CUTS))
