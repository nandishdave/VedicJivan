# -*- coding: utf-8 -*-
"""Candidate H - COMBUSTION (Astangata) as factor 16 vs the current 15-factor model.
A planet within the Sun's combustion orb is 'burnt' and loses power to give results.
Orbs (deg from Sun): Moon 12, Mars 17, Mercury 14, Jupiter 11, Venus 10, Saturn 15.
Fame hypothesis: famous have FEWER combust KEY planets (benefics + lagna/10th lords) —
their significators are free to deliver. Metrics:
  comb_count = # combust among the 6 (expect famous LOWER)
  comb_ben   = # combust natural benefics J/V/Me (expect LOWER)
  laglord_free / tenlord_free = lagna-lord / 10th-lord NOT combust (expect HIGHER)
  key_free   = laglord_free + tenlord_free
...
COMB = {"Moon": 12.0, "Mars": 17.0, "Mercury": 14.0, "Jupiter": 11.0, "Venus": 10.0, "Saturn": 15.0}
_BEN = ("Jupiter", "Venus", "Mercury")
MKEYS = ["comb_count", "comb_ben", "laglord_free", "tenlord_free", "key_free"]
...
def feats(p):
    ...
    # ---- CANDIDATE H: combustion ----
    sun_lon = P["Sun"]["longitude"]
    def is_combust(pl):
        if pl == "Sun" or pl not in COMB: return False
        sep = abs(P[pl]["longitude"] - sun_lon) % 360
        sep = min(sep, 360 - sep)
        return sep < COMB[pl]
    comb_count = float(sum(1 for q in COMB if is_combust(q)))
    comb_ben = float(sum(1 for q in _BEN if is_combust(q)))
    l1, l10 = SIGN_LORDS[ls], SIGN_LORDS[(ls + 9) % 12]
    laglord_free = 0.0 if is_combust(l1) else 1.0   # Sun-as-lord is never combust -> free
    tenlord_free = 0.0 if is_combust(l10) else 1.0
    key_free = laglord_free + tenlord_free
    strength = [comb_count, comb_ben, laglord_free, tenlord_free, key_free]
