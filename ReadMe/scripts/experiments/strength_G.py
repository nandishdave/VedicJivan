# -*- coding: utf-8 -*-
"""Candidate G - ISHTA PHALA (benefic result-giving capacity) as factor 16 vs the current
15-factor model. Ishta = sqrt(Uccha bala x Cheshta bala) [0-60] — the classical measure of
whether a planet gives AUSPICIOUS results (distinct from raw total strength). Read from the
engine's shadbala components ochcha_bala + chesta_bala. Metrics:
  ishta_lag / ishta_10 = Ishta of the lagna-lord / 10th-lord
  ishta_lords = mean of the two (self+career giving good results)
  ishta_avg   = mean Ishta of the 7 planets
  ishta_ben   = mean Ishta of the natural benefics J/V/Me
...
MKEYS = ["ishta_lag", "ishta_10", "ishta_lords", "ishta_avg", "ishta_ben"]
...
def feats(p):
    ...
    # ---- CANDIDATE G: Ishta Phala = sqrt(Uccha x Cheshta) ----
    def ishta(pl):
        if pl not in sb: return 30.0
        u = max(0.0, sb[pl].get("ochcha_bala", 30.0)); ch = max(0.0, sb[pl].get("chesta_bala", 30.0))
        return math.sqrt(u * ch)
    l1, l10 = SIGN_LORDS[ls], SIGN_LORDS[(ls + 9) % 12]
    ishta_lag = ishta(l1); ishta_10 = ishta(l10)
    ishta_lords = (ishta_lag + ishta_10) / 2.0
    ishta_avg = float(np.mean([ishta(q) for q in _C]))
    ishta_ben = float(np.mean([ishta(q) for q in _BEN]))
    strength = [ishta_lag, ishta_10, ishta_lords, ishta_avg, ishta_ben]
...
for i, k in enumerate(MKEYS):
    print(f"  {k:12} fam={FS[:,i].mean():6.2f} ord={RS[:,i].mean():6.2f}  soloAUC={auc(FS[:,i], RS[:,i]):.3f}")
print("\nbase 15-factor:            " + "  ".join(f"{k}={cv(F[CUTS[k]],R):.3f}" for k in CUTS))
for i, k in enumerate(MKEYS):
    Fn, Rn = add([i]); print(f"+ {k:12}          " + "  ".join(f"{ck}={cv(Fn[CUTS[ck]],Rn):.3f}" for ck in CUTS))
