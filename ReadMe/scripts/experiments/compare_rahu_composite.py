"""Add RAHU YOGA as a 9th factor (Rahu-dasha-years lived in peak x dispositor-
unafflicted) and re-run the composite: 79 famous vs 96 ordinary, before vs after."""
import json
import numpy as np
...
FEAT=["dasha","dhana","prosperity","raja","func_ben","av_1st","av_10th","d60","rahu_yoga"]
...
def feats(dob,tob,lat,lon):
    ...
    # RAHU YOGA: Rahu-dasha years LIVED in peak (20-50, elapsed by NOW) x dispositor-unafflicted
...
def report(cols,label):
    ...
    sign=np.sign(Xf.mean(0)-Xr.mean(0)); mu,sd=X.mean(0),X.std(0)+1e-9; cnt=(((X-mu)/sd)*sign>0).sum(1)
    order=np.argsort(-cnt); mr=np.median([list(order).index(i)+1 for i in range(len(Xf))])
    top=25; tf=sum(1 for k in order[:top] if k<len(Xf))
    print(f"  {label:26s} CV-AUC={auc(cvc,y):.3f}  stack {cnt[y==1].mean():.2f} vs {cnt[y==0].mean():.2f}  top{top}={tf} fam  fam-median {mr:.0f}/{len(y)}")
print()
report(list(range(8)),"8-factor (before)")
report(list(range(9)),"9-factor (+ Rahu Yoga)")
report([8],"Rahu Yoga ALONE")
