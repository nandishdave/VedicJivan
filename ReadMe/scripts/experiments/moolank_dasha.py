# -*- coding: utf-8 -*-
"""Do famous run their Moolank/Bhagyank planet's MAHADASHA in prime (ages 20-50)?
Descriptives (either / neither / count / coverage) famous vs ordinary + solo AUC
+ composite lift (full + confound-matched cuts). (225 vs 96)"""
...
def _md_overlap(dl, by, planet):
    """Years of ``planet``'s mahadasha(s) that fall inside [PA, PB]."""
    ov = 0.0
    for d in dl:
        if d["planet"] == planet:
            ov += max(0, min(int(d["end_date"][:4]) - by, PB) - max(int(d["start_date"][:4]) - by, PA))
    return ov
def feats(p):
    ...
    mp, bp = NUMP[mool], NUMP[bhag]
    mo, bo = _md_overlap(dl, by, mp), _md_overlap(dl, by, bp)
    both_same = (mp == bp)
    cov = mo if both_same else (mo + bo)          # union years (MDs never overlap each other)
    return f12, (1.0 if mo > 0 else 0.0), (1.0 if bo > 0 else 0.0), min(cov, PB - PA) / (PB - PA), by, (68 <= b["lon"] <= 98 and 6 <= b["lat"] <= 37)
FR = [feats(p) for p in FAM]; RR = [feats(p) for p in ORDD]
F12 = np.array([x[0] for x in FR]); R12 = np.array([x[0] for x in RR])
FMo = np.array([x[1] for x in FR]); RMo = np.array([x[1] for x in RR])
FBo = np.array([x[2] for x in FR]); RBo = np.array([x[2] for x in RR])
FCov = np.array([x[3] for x in FR]); RCov = np.array([x[3] for x in RR])
...
Fcount = FMo + FBo; Rcount = RMo + RBo
Feither = (Fcount > 0).astype(float); Reither = (Rcount > 0).astype(float)
print(f"famous={len(FR)} ordinary={len(RR)}   prime = ages {PA}-{PB}\n")
print("DESCRIPTIVE (share of each group):")
print(f"  Moolank MD touches prime:        famous {FMo.mean()*100:4.0f}%   ordinary {RMo.mean()*100:4.0f}%")
print(f"  Bhagyank MD touches prime:       famous {FBo.mean()*100:4.0f}%   ordinary {RBo.mean()*100:4.0f}%")
print(f"  EITHER MD in prime:              famous {Feither.mean()*100:4.0f}%   ordinary {Reither.mean()*100:4.0f}%")
print(f"  NEITHER MD in prime:             famous {(1-Feither).mean()*100:4.0f}%   ordinary {(1-Reither).mean()*100:4.0f}%")
print(f"  BOTH MDs in prime:               famous {(Fcount==2).mean()*100:4.0f}%   ordinary {(Rcount==2).mean()*100:4.0f}%")
print(f"  mean prime-coverage by the two:  famous {FCov.mean()*100:4.0f}%   ordinary {RCov.mean()*100:4.0f}%\n")
...
for name, Ff, Rf in (("EITHER (binary)", Feither, Reither), ("COUNT (0-2)", Fcount, Rcount), ("COVERAGE (0-1)", FCov, RCov)):
    F13 = np.column_stack([F12, Ff]); R13 = np.column_stack([R12, Rf])
    print(f"{name:16} solo-AUC={auc(Ff,Rf):.3f}  |  +composite full={cv(F13,R13):.3f}  >=0={cv(F13[mask0],R13):.3f}  >=1940={cv(F13[mask40],R13):.3f}  >=1955={cv(F13[mask55],R13):.3f}")
